...
const metricsEvts = require('./traces/metrics-evts');
...
function getFilenamePrefix(results) {
  ...
  const date = new Date(results.generatedTime) || new Date();
  ...
  return passNames.reduce((chain, passName) => {
    ...
    return chain.then(_ => artifacts.requestScreenshots(trace))
      .then(screenshots => {
        ...
        traceData.traceEvents = metricsEvts.addMetrics(traceData.traceEvents, results.audits);
